...
def gelu(x):
    cdf = 0.5 * (1.0 + tf.tanh(
        (np.sqrt(2 / np.pi) * (x + const_gelu * tf.pow(x, 3)))))
    return x * cdf
...
def lstm_model(x_val, y_val, epochs_num, look_back, train_samples, n_steps_out, trail_num, cnrtl_val) :
    ...
    model.add(TimeDistributed(Conv1D(filters=cnrtl_val * 1, kernel_size=1, activation=gelu),
                              input_shape=(x_val.shape[1], look_back, n_steps_out)))
    if cnrtl_val > 2:
