...
from Main_Model_v6_5 import lstm_model
...
def get_nse(comp_name):
    ...
    # Adding a ok value
    tmp_lst = []
    for row in range(len(recv_data['Date'])):
        tmp_lst.append('ok')
    stock_data['s'] = tmp_lst

    # Converting from Timestamp to UNIX Times
    tmp_lst = []
    for row in recv_data.itertuples():
        unixtime = time.mktime(row.Date.timetuple())
        tmp_lst.append(unixtime)
    stock_data['t'] = tmp_lst
...
class Trader:
    ...
    def back_test(self, data):
        ...
        today_data = X_data[len(X_data)-1:]
